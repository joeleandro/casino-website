...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/products", async (req, res) => {
    try {
      console.log("🔍 API: Recebendo requisição /api/products");
      console.log("📌 DATABASE_URL configurada:", process.env.DATABASE_URL ? "Sim" : "Não");
      
      const category = req.query.category as string;
      const search = req.query.search as string;
      
      console.log("🔍 Parâmetros:", { category, search });
      ...
      console.log(`✅ Query bem-sucedida. Encontrados ${result.length} produtos.`);
